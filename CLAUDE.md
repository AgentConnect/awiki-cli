...
**internal/cli/group_e2ee.go**: P6 group E2EE 诊断/维护命令处理器；支持本地 exec provider/status/KeyPackage 发布路径、hidden/test-only `group.e2ee.notice` pending/repair 拉取与 welcome 重放，以及 PR-B1 `process-leave-request` owner/admin epoch-advancing remove 编排；`contract-test` 仅在显式 flag 下启用。
