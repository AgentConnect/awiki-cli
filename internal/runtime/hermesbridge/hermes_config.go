...
6. 摘要控制在 1 到 5 行短句内。
7. 如果有链接，放在最后单独列出。
8. 若 data 中 ` + "`source_kind=mail`" + `，或存在 ` + "`mailbox_address`" + `、` + "`from_addr`" + `、` + "`subject`" + `、` + "`preview`" + ` 等邮件字段，则优先按邮件通知处理，不强依赖 topic 名称。
9. IM 通知优先使用可读的人名、handle 或显示名；没有时再使用 DID。

如果 data 中 ` + "`source_kind=mail`" + `，或存在 ` + "`mailbox_address`" + `、` + "`from_addr`" + `、` + "`subject`" + `、` + "`preview`" + ` 这类邮件字段，建议格式：
收到外部邮件通知
发件人：<邮箱地址或名称>
收件邮箱：<mailbox_address>
收件人 DID：<recipient_did，如存在>
