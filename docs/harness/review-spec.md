...
|---|---|---|---|
| `docs/plan/phase-0/implementation-constraints.md` | 冻结约束 | **最高优先级** | canonical 顶级命令、`group` 顶级归属、全局 flags、输出 envelope、错误码/退出码、`AWIKI_CLI_WORKSPACE_HOME_DIR`、`config.json`、单根目录工作区路径、`owner_did`、Phase 边界全部以此为准。 |
...
| `internal/config/config.go` | workspace / env / config 解析 | 仅支持 `AWIKI_CLI_WORKSPACE_HOME_DIR`，统一读取 `config.json`，并在发现旧环境变量或旧 `config.yaml` 时直接报错；同时保持单根目录工作区路径与 legacy path 检测。 |
...
- [ ] 配置优先级是否仍遵循：`flag > config.json > default`
