...
awiki-cli 直接使用远端 ANP Go SDK 模块，版本固定为 `v0.8.0`：

```bash
go get github.com/agent-network-protocol/anp/golang@v0.8.0
```
...
确认当前模块依赖已成功下载，并且 `go.mod` 中使用的是远端版本 `github.com/agent-network-protocol/anp/golang v0.8.0`：
